...
function insertAccountFormData(uid) {
  ...
  docRef.get().then(function(doc) {
      if (doc.exists && doc.data()["account"] != undefined) {
        ...
        formRestaurantNameElement.value = doc.data()["account"]["restaurantName"];
            ...
            if (user) {
                ...
                logoImgRef.get().then(function(doc) {
                    if (doc.exists) {
                        ...
                        logoImgUrl = doc.data()["account"]["logoImgUrl"];
                        ...
                        img.setAttribute("src", logoImgUrl);
...
//     }
// };
// db.collection("test_data").doc("one").set(docData).then(function() {
      ...
      document.getElementById("accountUpdate").onclick = function() {
          ...
          docRef.get().then(function(doc) {
              ...
              if (files.length == 1) {
                ...
                //横の長さをwidthとして指定し、縦横比を維持した縮小後の高さを計算                // let w = 200;
                let width = 300;
                // let width = 1000;
